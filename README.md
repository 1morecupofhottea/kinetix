...
kinetix key create --name "pi" --owner me        # prints sk-kinetix-... once
...
`doctor`, `status`, `password`). The admin CLI writes the SQLite control plane
...
### Alternative: build from a checkout with a bootstrap file

For a reproducible local build you can still drive startup from a TOML file:

```bash
cp .env.example .env                 # optional; set KINETIX_MASTER_KEY etc.
cp config.toml.example config.toml   # set your upstream api_key
./scripts/build-dashboard.sh         # builds the dashboard, then the binary that embeds it
./target/release/kinetix             # serves; seeds config.toml on first run
```

On first start Kinetix seeds the database from `config.toml` and prints the generated
virtual key **once** to the logs. Afterwards the database is authoritative.
...
- `kinetix init` (or the first `kinetix serve`) generates an admin password and
  prints it **once**; it is stored as a hash (`~/.config/kinetix/admin_password.hash`
  and the `admin_password_hash` DB setting). Change it with
  `kinetix password set` or from the dashboard.
- The dashboard logs in with that password and receives an httpOnly session
  cookie carrying an in-memory session token. Sessions live only in process
  memory with a TTL (`KINETIX_SESSION_TTL_MINUTES`, default 720), so **a server
  restart invalidates every session** and a browser must log in again.
...
- CLI/tooling may instead send `x-kinetix-admin-token` carrying a live session
  token or the current raw admin password. A raw credential is never accepted
  from the cookie (NFR-3.14 — credentials stay write-only).
...
proxy on localhost behind cloudflared (see `deploy/README.md`).
